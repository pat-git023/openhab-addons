...
public class OpenEMSHandler extends BaseThingHandler {
    ...
    private void publishChannelIfLinked(ChannelUID channelUID) {
        ...
        try {
            ...
            switch (channelID) {
                case META_VERSION:
                    state = new StringType(dataValue.getValue());
                    break;
                case SUM_GRID_MODE:
                case SUM_STATE: {
                ...
                case SUM_ESS_SOC:
                case SUM_ESS_ACTIVE_POWER_L1:
                case SUM_ESS_ACTIVE_POWER_L2:
                case SUM_ESS_ACTIVE_POWER_L3:
                case SUM_GRID_ACTIVE_POWER:
                case SUM_GRID_ACTIVE_POWER_L1:
                case SUM_GRID_ACTIVE_POWER_L2:
                case SUM_GRID_ACTIVE_POWER_L3:
                case SUM_PRODUCTION_ACTIVE_POWER:
                case SUM_PRODUCTION_DC_ACTUAL_POWER:
                case SUM_CONSUMPTION_ACTIVE_POWER:
                case SUM_CONSUMPTION_ACTIVE_POWER_L1:
                case SUM_CONSUMPTION_ACTIVE_POWER_L2:
                case SUM_CONSUMPTION_ACTIVE_POWER_L3:
                case SUM_ESS_ACTIVE_POWER:
                case SUM_ESS_ACTIVE_CHARGE_ENERGY:
                case SUM_ESS_ACTIVE_DISCHARGE_ENERGY:
                case SUM_GRID_BUY_ACTIVE_ENERGY:
                case SUM_GRID_SELL_ACTIVE_ENERGY:
                case SUM_PRODUCTION_AC_ACTIVE_ENERGY:
                case SUM_PRODUCTION_DC_ACTIVE_ENERGY:
                case SUM_CONSUMPTION_ACTIVE_ENERGY:
                case SUM_ESS_APPARENT_POWER:
                case SUM_GRID_MAX_ACTIVE_POWER:
                case SUM_ESS_CAPACITY:
                case SUM_CONSUMPTION_MAX_ACTIVE_POWER:
                case SUM_GRID_MIN_ACTIVE_POWER:
                case SUM_ESS_DC_DISCHARGE_ENERGY:
                case SUM_PRODUCTION_MAX_DC_ACTUAL_POWER:
                case SUM_ESS_DISCHARGE_POWER:
                case SUM_ESS_DC_CHARGE_ENERGY:
                case SUM_PRODUCTION_MAX_ACTIVE_POWER:
    ...
    private HashMap<String, OpenEMSData> fetchData() {
        ...
        try {
            ...
            var auth = "Basic " + Base64.getEncoder()
    ...
    public OpenEMSData[] parseDataFromStream(InputStream stream){
        try {
            return new Gson().fromJson(new InputStreamReader(stream), OpenEMSData[].class);
        } catch (Exception e) {
            logger.warn("Could not parse json", e);
        }
        return new OpenEMSData[]{};
    }
